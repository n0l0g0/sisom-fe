    ...
    const run = async () => {
      try {
        ...
        const [contractsRes, roomsRes] = await Promise.all([
          api.getContracts(showInactive ? undefined : true, q),
          api.getRooms()
        ]);
        if (cancelled) return;
        ...
        <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-2 text-sm border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full md:w-64"
              defaultValue={q || ''}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
