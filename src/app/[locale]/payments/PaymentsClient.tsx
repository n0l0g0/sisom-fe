...
function PaymentsClientInner({ room, status, month, year }: { room: string; status: string; month: number; year: number }) {
  ...
  useEffect(() => {
    let cancelled = false;
    api
      .getPayments(room || undefined, status || undefined, month || undefined, year || undefined)
      .then((p) => {
        if (cancelled) return;
        setPayments(p);
      })
  ...
  return (
    <div className="space-y-8 fade-in">
      ...
      <Filters defaultRoom={room} defaultStatus={status} defaultMonth={month} defaultYear={year} />
